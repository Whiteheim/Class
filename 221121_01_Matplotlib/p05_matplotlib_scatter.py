...
# 동일한 순서로 난수를 발생시키기 위해 'seed'라는 것을 난수에 부여
np.random.seed(0)
...
# colors = np.random.rand(n)      # rgb, hexcode(16진법)의 색상을 지정
                                # # 0 ~ 1까지의 숫자를 임이의 색으로 지정하게됨
...
#
# # alpha : 마커의 투명도 (0 ~ 1) / cmap : 컬러맵에 해당하는 문자열을 지정 (테마 같은 개념)
